...
impl MathExp {
    ...
    fn push_to_token(&mut self, t: token::Token) {
        ...
        if last_token.is_none() {
            ...
            if !matches!(t, token::Token::Operation(_))
                || matches!(t, token::Token::Operation(token::Op::ParenLeft))
            {
        ...
        let allow_insert = match last_token {
            ...
            token::Token::Operation(token::Op::ParenRight) => match t {
                token::Token::Operation(token::Op::ParenLeft) => false,
        ...
        if allow_insert {
            ...
            self.update_output(
                format!("Токен {} не может быть добавлен после {}", t, last_token).as_str(),
            );
        }
    ...
    fn pop_buffer(&mut self) -> bool {
        ...
        if let Ok(val) = self.buffer.parse::<f64>() {
            if val.is_sign_negative() {
                self.tokens
                    .push(token::Token::Operation(token::Op::ParenLeft));
                self.tokens.push(token::Token::Operand(val));
                self.tokens
                    .push(token::Token::Operation(token::Op::ParenRight));
                self.buffer.clear();
            } else {
                self.tokens.push(token::Token::Operand(val));
                self.buffer.clear();
    ...
    pub fn pop(&mut self) {
        if self.buffer.is_empty() {
            ...
            if matches!(self.tokens.last(), Some(token::Token::Function(_))) {
                self.tokens.pop();
            }
        } else {
        ...
        match rpn {
            ...
            Ok(tokens) => {
                ...
                self.output = if let Some(t) = stack.pop() {
                    ...
                    "Ошибка вычисления".to_string()
                };
                println!("=> {}", self.output);
                self.add(self.output.clone().as_str());
            }
        }
    }
}
...
fn yard(input: &Vec<token::Token>) -> Result<Vec<token::Token>, String> {
    ...
    for token in input {
        match token {
            ...
            token::Token::Function(_f) => stack.push(token.clone()),
            token::Token::Operation(token::Op::ParenLeft) => stack.push(token.clone()),
            token::Token::Operation(token::Op::ParenRight) => loop {
                if let Some(last_token_in_stack) = stack.pop() {
                    match last_token_in_stack {
                        token::Token::Operation(token::Op::ParenLeft) => {
                            break;
                        }
                        _ => output.push(last_token_in_stack.clone()),
                    }
                } else {
